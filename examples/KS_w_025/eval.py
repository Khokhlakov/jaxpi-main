...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    # ── post-hoc spectral filter
    # Applies the model's own dealiasing operator to the finished trajectory,
    # purely to check how much of the visible high-frequency jitter is a
    # cosmetic artifact of the network's unconstrained-above-cutoff band
    logging.info("Applying post-hoc dealiasing filter for comparison...")
    dealias_batched = jax.vmap(jax.vmap(model.solver._dealias))  # maps over (ics, time)
    x_pred_full_filt = jnp.fft.irfft(
        dealias_batched(jnp.fft.rfft(x_pred_full, axis=-1)),
        n=N_loaded, axis=-1,
    )
    # ── 4b. Same trajectory plots, but on the filtered rollout ──────────────
    for ic_idx in range(min(total_plots, num_ics)):
        logging.info(f"--- Generating filtered summary for IC {ic_idx} ---")

        save_path = os.path.join(
            workdir, "figures", config.wandb.name,
            f"trajectory_summary_ic_{ic_idx}_dealiased.pdf"
        )

        _plot_trajectory_summary(
            t_ax=t_ax,
            x_true=np.array(u_test[ic_idx]),
            x_est=np.array(x_pred_full_filt[ic_idx]),
            ic_idx=ic_idx,
            test_windows=test_windows,
            pts_pw=pts_pw,
            save_path=save_path,
    ...
    # ── 5b. Same batch error analysis, but on the filtered rollout ──────────
    err_filt = x_pred_full_filt - u_test
    l2_rel_filt = np.array(
        jnp.linalg.norm(err_filt, axis=-1) / (jnp.linalg.norm(u_test, axis=-1) + 1e-12)
    )
    overall_mean_l2_filt = np.mean(l2_rel_filt, axis=0)

    _plot_batch_l2_over_time(
        t_ax=t_ax,
        overall_mean_l2=overall_mean_l2_filt,
        save_path=os.path.join(
            workdir, "figures", config.wandb.name, "batch_l2_error_analysis_dealiased.pdf"
        ),
    )
    logging.info(f"filtered batch L2 error plot saved to: batch_l2_error_analysis_dealiased.pdf")
