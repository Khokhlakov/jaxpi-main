...
def get_config():
    ...
    # Weights & Biases
    # resume from result of config 17 with grad_norm and adam optimizer
    config.wandb = wandb = ml_collections.ConfigDict()
    wandb.project = "PI-UDON-L96-small-separated"
    wandb.name = "sep_test_22" 
    wandb.tag = None
    ...
    arch.fourier_emb = ml_collections.ConfigDict({"embed_scale": 3, "embed_dim": 256})
    ...
    training.max_additions = 1

    # Weighting
    config.weighting = weighting = ml_collections.ConfigDict()
    weighting.scheme = "grad_norm"
    weighting.init_weights = ml_collections.ConfigDict({"ics": 1000.0, "res": 10.0}) 
