...
def get_config():
    ...
    # Weights & Biases
    # T3 with data addition
    config.wandb = wandb = ml_collections.ConfigDict()
    wandb.project = "PI-UDON-L96-small-separated"
    wandb.name = "sep_test_7" 
    wandb.tag = None

    # Arch 
    config.arch = arch = ml_collections.ConfigDict()
    arch.arch_name = "DeepONet"
    arch.num_branch_layers = 3
    arch.num_trunk_layers = 3
    arch.hidden_dim = 128
    arch.branch_input_dim = 6
    arch.out_dim = 6
    arch.activation = "tanh"
    arch.periodicity = None
    arch.fourier_emb = ml_collections.ConfigDict({"embed_scale": 5, "embed_dim": 128})
    ...
    training.batch_size_per_device = 4096#16384
