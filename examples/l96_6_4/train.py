...
def _load_rollout_pool(max_additions: int, num_initial_ics: int,
                       num_vars: int, mat_path="data/train_rollouts_025.mat") -> jnp.ndarray:
    """
...
def train_and_evaluate(config, workdir: str):
    ...
    logging.info(f"x_ref_eval_all: {x_ref_eval_all.shape}")
    logging.info(f"u_ref_eval_all: {u_ref_eval_all.shape}")
    logging.info(f"t_star_all: {t_star_all.shape}")

    u_ref_eval = u_ref_eval_all[0:trajs_per_window, :]                  # shape (100, N)
    ...
    if augmentation_scheme == "file":
        ...
        u0_pool    = _load_rollout_pool(
            ...
            "data/"+config.training.get("augmentation_file_name", "train_rollouts_025.mat")
        )
    ...
    if config.saving.get("restore_checkpoint", False):
        ...
        # Pre-allocate lists to store the slices
        u_ref_list = []
        x_ref_list = []
        ...
        for k in range(max_additions + 1):
            ...
            # 1. Construct u_ref slice (Extract the state at the start of this time window)
            # Shape of u_slice: (trajs_per_window, 40)
            u_slice = x_ref_eval_all[0:trajs_per_window, t_start, 0:40]
            u_ref_list.append(u_slice)
            
            # 2. Construct x_ref slice (Extract the full trajectory for this time window)
            # Shape of x_slice: (trajs_per_window, time_steps, 40)
            x_slice = x_ref_eval_all[0:trajs_per_window, t_start:t_end, 0:40]
            x_ref_list.append(x_slice)

        # 3. Concatenate the lists along the first axis (axis=0)
        u_ref_eval = jnp.concatenate(u_ref_list, axis=0)
        x_ref_eval = jnp.concatenate(x_ref_list, axis=0)
