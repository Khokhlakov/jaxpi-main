...
def get_config():
    ...
    # Weights & Biases
    # Uses nn trained by config 2 11
    # Start with null P_0, let sigma_obs = 0
    # Before window aware surrogate
    # No observation
    config.wandb = wandb = ml_collections.ConfigDict()
    wandb.project = "PI-UDON-L96-n40-f6-ics-2"
    wandb.name = "test3_sigma_obs_0_2"
    wandb.ckpt_name = "test2_11"
    wandb.tag = None
    ...
    config.ekf.sigma_obs    = 0.0
    config.ekf.sigma_proc   = 0.1 # Unused in EnKF
    config.ekf.P0_sigma     = 0.0 
    config.ekf.dynamic_vars = False # True -> randpick vars

    config.ekf.dt_fine   = 0.01
    config.ekf.dt_obs    = 7.5
    # dt_fine must divide dt_obs and config.dt_window

    # EnKF settings
    config.enkf = ml_collections.ConfigDict()
    config.enkf.sigma_model = 0.005
    config.enkf.N_ens       = 200
