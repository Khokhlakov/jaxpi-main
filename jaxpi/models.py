...
def _create_optimizer(config):
    if config.decay_schedule == "Exponential":
        if config.optimizer == "Adam":
            ...
            tx = soap(
                learning_rate=lr, b1=config.beta1, b2=config.beta2, weight_decay=0.0, precondition_frequency=10
                )
        ...
        if config.optimizer == "Soap":
            tx = soap(learning_rate=lr, b1=config.beta1, b2=config.beta2, eps=config.eps, precondition_frequency=2)
...
def _create_train_state(config):
    ...
    if config.arch.arch_name in ["DeepONet", "ModifiedDeepONet"]:
        u = jnp.ones(arch.branch_input_dim)
        y = jnp.ones(config.input_dim - arch.branch_input_dim)
        params = arch.init(random.PRNGKey(config.seed), u, y)
    else:
